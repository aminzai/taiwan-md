...
**開工（隔離優先 = default）**：多檔 / 跑 build / 長任務第一動作開 worktree — 用 `bash scripts/tools/semiont-worktree.sh new <title>`（**會 symlink node_modules + .env + .credentials**，讓 husky / lint-staged prettier pre-commit 跑得動；raw `git worktree add` 不 symlink → prettier ENOENT、禁 `--no-verify` 下 commit 卡死，2026-06-19 inbox-distill 踩過），讓 commit 污染結構性不可能（#9）。落地走 ff-push 不走 PR（worktree ≠ PR）：`semiont-worktree.sh ship`（fetch + rebase origin/main + push HEAD:main + 自毀）或手動 `git fetch && git rebase origin/main && git push origin HEAD:main`。用完 `worktree-gc.sh` 驗無丟工作再刪（不累積滯留）。session 啟動跑 `check-parallel-actor.sh` 知道誰在跑（#57）。
...
**`<描述>` 寫人話**（§11.4 電報腔紀律，2026-06-28）：標題跟內文都寫成一個人會寫的句子，不要壓成縮寫與斜線數據流（`vitals 825 / 7d=+27 / immune=50 chronic 第 5 cycle` 這種電報腔）。完整生命徵象、週期計數、趨勢數字放當天 memory 檔，commit 只講「改了什麼、為什麼」。紀律 canonical → [MANIFESTO §11.4](docs/semiont/MANIFESTO.md)。
