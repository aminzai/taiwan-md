...
- **`tw-flow`（Sankey／流向）— 唯一有明確觸發條件的候選**：§二 十五類裡只有「流向」這一列沒有模組。台灣題材裡它不稀有（半導體供應鏈、移工來源到產業、能源結構轉換、預算流向、人口遷移），但 2026-08-02 量到十九個模組已有七個零真實使用，這種狀態下量到的需求是假的。**觸發條件：投影候選清單累積 ≥3 篇文章標出流向關係，才動手做。** 在那之前用 `tw-stack` 兩階段或 `tw-heatmap` 流量矩陣替代（[設計報告 §三](../../reports/design-viz-adoption-2026-08-02.md)）。
