...
| 中華台北         | **チャイニーズ・タイペイ／中華台北** | 五輪、IOC、APEC、WHO 等公式機関のみ | 地の文の既定主語として（「台湾」に書き換える） | 地の文の主語は既定で「台湾」。大会脈絡の初出のみ「台湾（中華台北の名義で出場）」と注記し、以降は「台湾」のみ。直接引用／メダル公式集計／団体の正式名称／名称そのものの解説は中華台北のまま可（[EDITORIAL §六 賽事脈絡的稱名](../EDITORIAL.md)、2026-09-05 OBSERVER-QUEUE #41） |
