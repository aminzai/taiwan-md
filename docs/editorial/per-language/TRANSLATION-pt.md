...
| 中華台北        | **Taipé Chinesa**                                                                           | Só contexto COI/Jogos Olímpicos/APEC/OMS                                     | Como sujeito padrão da prosa (reescrever como `Taiwan`)                                    | Sujeito padrão sempre `Taiwan`. Na primeira menção do contexto competitivo em um artigo, anotar `Taiwan (competindo como Taipé Chinesa)`; depois, apenas `Taiwan`. `Taipé Chinesa` permanece em citações diretas, quadro oficial de medalhas, nome próprio de organização, ou trechos que explicam a designação em si ([EDITORIAL §六 賽事脈絡的稱名](../EDITORIAL.md), 2026-09-05 OBSERVER-QUEUE #41) |
