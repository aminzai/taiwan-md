...
| 輸出         | append SPORE-INBOX entry      | post Threads + X + spore-db add-spore | 同左                     |
...
| 同篇文章上次孢子距今 < 2 週（`spore-db.py last-spore`） | per SPORE-PIPELINE §排除規則       |
...
```bash
git add docs/factory/spore-log.json docs/factory/SPORE-INBOX.md docs/factory/SPORE-HARVESTS/ docs/semiont/memory/ docs/semiont/diary/ public/article-images/
...
quality_gate:
  - spore-log.json 新 id appended (除非 intake gap skip) OR LESSONS-INBOX entry 增加 ≥ 1
