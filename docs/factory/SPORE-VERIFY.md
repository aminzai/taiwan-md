...
| 配圖 (local default v3.6)     | SHIP       | 所有 spore             | `bash scripts/tools/make-spore.sh` (no `--prod`)                                 | —                                                            | 必跑           |
| 方形圖 AI 自檢 (v3.6)         | SHIP       | 所有 spore             | Read tool / preview_eval 視覺看圖內容對                                          | 重抓不發                                                     | 必跑           |
| CI/CD wait gate (v3.7)        | SHIP       | 所有 spore (post 前)   | `curl -sf URL \| grep title-keyword` poll 60s × **60min cap + 30min soft alert** | **routine: defer to next cycle** / manual: escalate observer | 必跑           |
| Routine auto-decisions (v3.7) | SHIP       | `SPORE_ROUTINE_MODE=1` | Platform Threads-only / Tier 1b / single-version / image plugin / zh-only        | 退回 manual mode                                             | 條件 (routine) |
...
**「必跑」15 條 / 「條件」6 條**（v3.7 將 CI/CD wait gate 升 60min cap + defer fallback，新增 Routine auto-decisions 條件 gate）。
...
**＋「重」當抽象份量隱喻**（2026-06-04 哲宇 callout）：`很重／最重的一刻／更重／沉重／沈重／份量／分量`——AI 愛把「意義／份量／重要性」寫成物理的「重」。plugin 用 regex + 負向預看抓（排除「很重要／很重視／很重大／嚴重／尊重／體重／重新／重點」等複合詞），逐處 WARN。口語替代：把抽象的「重」改成具體後果或畫面（「最重的一刻」→ 直接寫那一刻發生什麼／為什麼忘不掉；「份量很重」→「壓得人喘不過氣」）。物理重量（這箱子很重）例外。
