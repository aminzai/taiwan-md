...
| 🤝 分靈節點   | 自己的機器跑一條 cron，每天接一件工單以 PR 回主庫 | Lv.0 起（第一個 node PR merged 後可接寫作工單） |

> **分靈節點是角色不是階梯**：它不改變任何 GitHub 權限——節點的輸出永遠止於一個 PR，走的是 Lv.1 contributor 同一條免疫路徑。掛這個標籤只代表「這個人的機器也在幫忙」。完整 SOP：[CONTRIBUTOR-NODE-PIPELINE.md](CONTRIBUTOR-NODE-PIPELINE.md)（2026-07-25）。
