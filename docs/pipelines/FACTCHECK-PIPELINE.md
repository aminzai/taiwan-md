...
# FACTCHECK-PIPELINE.md — 完整事實查核方法論（SSOT）v2.0

> **第一性原理**：每一個 footnote、每一個直接引語、每一個具體數字，都必須通過 **「URL 活著 → 媒體真實發過 → 描述準確 → claim 跟 source 逐字對應」** 四關。少任一關 = 系統性 trust leak。
>
> v2.0 設計理由：對齊 [REWRITE-PIPELINE v5.0](REWRITE-PIPELINE.md) + [MAINTAINER-PIPELINE v2.0](MAINTAINER-PIPELINE.md) spine restoration 範式。修補 v1.1 結構問題：(1) 缺 ASCII spine + Quick/Full Mode 對照；(2) Hard Gate 散在 Phase 1-6 無集中索引；(3) Top 5 最常忘沒提取。
...
```
╭──────────────────────────────────────────────────────────────────────────╮
│         FACTCHECK-PIPELINE 6 phase + Quick/Full Mode                     │
│                                                                          │
│   🧭 核心命題：4 關 trust leak（URL / 媒體 / 描述 / claim verbatim）     │
│                                                                          │
│   🪜 Article Tier 制                                                     │
│            ├── A 級: People / 敏感 / 政治 / 紀錄片（≥50 fn / ≥3000 字） │
│            │       → Full Mode 90-180 min / 25+ WebFetch / Phase 1-6     │
│            ├── B 級: 一般深度文（10-50 fn）                              │
│            │       → 介於 / Phase 1, 3, 4 跑 + 2 採樣抽取                │
│            └── C 級: Hub / 短修正 / 翻譯 / Food / Geography 軟文         │
│                  → Quick Mode 15-30 min / 5-10 WebFetch / Phase 3-4 採樣 │
│                                                                          │
│   ──── Phase 1-6 主流程 ──────────────────────────────────────          │
│                                                                          │
│   Phase 1: SCOPE & BUDGET ──→ 5 step                                     │
│            ├── 判 article tier + atom 採樣目標 + WebFetch 預算           │
│            ├── 決定 spawn 策略（自跑 / general-purpose / Explore）       │
│            └── 指定輸出位置（research 檔 append / LESSONS-INBOX）        │
│              ↳ Hard gate: A 級無 research 檔 → 暫停先補 Stage 1          │
│                                                                          │
│   Phase 2: ATOMIC DECOMPOSITION ──→ 8 atom 類抽取                        │
│            ├── 時間 / 地點 / 動作 / 引語 / 數字 / 人名 / 機構 / 物件     │
│            └── 排成 atom 表（每 atom 一行 + footnote + URL 對應）        │
│              ↳ Hard gate: A 級 atom 表 < 50 行 → 重抽過淺                │
│                                                                          │
│   Phase 3: SOURCE AUTHORITY AUDIT ──→ 4 維度 × N footnote                │
│            ├── URL_resolves / source_real / desc_accurate / claim_matches│
│            └── WebFetch 中文 prompt 強制（避免英文 summary 污染）        │
│              ↳ Hard gate: A 級每 footnote 必有 4 維度報告                │
│                                                                          │
│   Phase 4: CLAIM VERBATIM CHECK ──→ Ctrl-F 逐字驗證                      │
│            ├── 引語 verbatim / 數字 base / 引號 third-person flip        │
│            └── over-citing 偵測（一 footnote 綁 N 個高風險 atom）        │
│              ↳ Hard gate: verbatim 不靠「看起來像」必 Ctrl-F             │
│                                                                          │
│   Phase 5: CROSS-CLAIM CONSISTENCY ──→ 算術 / 時序 / 互引                │
│            ├── 算術 sanity（總額 vs 分項）                               │
│            ├── 時序矛盾（A 在 B 之前 vs B 在 A 之前）                    │
│            └── 互引一致性（X claim Y vs Y claim X）                      │
│              ↳ Hard gate: 算術 base 數字必 cross-source 驗證             │
│                                                                          │
│   Phase 6: TRIAGE & APPLY ──→ ✅/⚠️/❌/🔴 分類處置                       │
│            ├── ✅ PASS / ⚠️ SOFT-FIX（改描述）                           │
│            ├── ❌ HARD-FIX（換源或刪 claim）                             │
│            └── 🔴 DEAD-LINK（必換源 + LESSONS append）                   │
│              ↳ Hard gate: 任一 🔴 / ❌ 不處置 不 ship                    │
│                                                                          │
│   ✅ Audit 完成                                                          │
│                                                                          │
│   ──── 跨 pipeline boundary ─────────────────────────                   │
│   → REWRITE Stage 3.3 Quick Mode 觸發（每篇 ship 前）                   │
│   → REWRITE Stage 3.5 Full Mode hallucination audit（A 級）             │
│   → REWRITE Stage 3.6 STORY ATOM AUDIT（場景原子驗證）                  │
│   → MAINTAINER §Step 3.4 footnote source authority audit（外部 PR）     │
╰──────────────────────────────────────────────────────────────────────────╯
```
...
| -------------------- | ---------- | ------------------------------------------ | --------------------------------------- | ---------------------------- |
| Article tier 判定    | Phase 1    | 所有 audit                                 | manual（A/B/C 三 tier 規則）            | 隨意亂跑                     |
| A 級必有 research 檔 | Phase 1    | A 級 article                               | `ls reports/research/YYYY-MM/{slug}.md` | 暫停先補 REWRITE Stage 1     |
| Atom 採樣目標        | Phase 1    | A: 100% / B: ~50 / C: 5-10                 | manual 計算                             | 重列                         |
| WebFetch 預算        | Phase 1    | A: 25+ / B: 15+ / C: 5+                    | manual 跟蹤                             | 升 tier 或減 scope           |
| 輸出位置指定         | Phase 1    | A/B append research 檔 / C → LESSONS-INBOX | manual                                  | 隨手寫 commit message        |
| Atom 表 ≥ 50 行      | Phase 2    | A 級 article                               | manual count                            | 重抽（過淺）                 |
| 8 atom 類涵蓋        | Phase 2    | 所有 audit                                 | grep 半自動                             | 補抽                         |
| Footnote 4 維度報告  | Phase 3    | A 級每 footnote                            | manual + WebFetch                       | 缺一不進 Phase 4             |
| WebFetch 中文 prompt | Phase 3    | 中文網站                                   | manual prompt 規範                      | 英文 summary 污染 → 重 fetch |
| Verbatim Ctrl-F      | Phase 4    | 所有引語 atom                              | Ctrl-F vs source                        | 「看起來像」一律 fail        |
| Over-citing 偵測     | Phase 4    | 多 atom 同 footnote                        | manual scan                             | mismatch atom 拆/換源        |
| 算術 cross-source    | Phase 5    | 數字 atom                                  | python algebra + 二源驗                 | base 數字錯誤 → HARD-FIX     |
| 時序矛盾             | Phase 5    | 時間 atom 互引                             | manual timeline                         | 衝突 → 改寫                  |
| 🔴 DEAD-LINK 處置    | Phase 6    | URL 404/403                                | curl -sI                                | 必換源 + LESSONS append      |
| ❌ HARD-FIX 處置     | Phase 6    | claim mismatch                             | per Phase 3-4 audit                     | 不 ship until fix            |

---

## ⚠️ Top 5 最常忘的 step

> 從 2026-04-28 沈伯洋 audit + LESSONS 抽 friction 最高的 5 條。

1. **Phase 4 verbatim 用 Ctrl-F 不是「看起來像」** — 引語 atom 必須逐字對 source URL Ctrl-F，AI 看起來「相似」會放過 quote re-paraphrase / third-person flip
2. **Phase 3 dead-link 必跑 WebFetch ≥ 3 footnote** — `URL_resolves` 第一維度，404/403 是最容易發現的，但因為 lazy 常常 skip
3. **Phase 2 atom 採樣率對 A 級 100% 全列** — 不是「抓重點」，是「全列 30 引語 + 20 數字 + 15 人名 + 15 機構 + 10 地點 + 10 物件 = ~100 atoms」
4. **WebFetch 對中文網站必須中文 prompt** — AI 預設返回英文 paraphrase，必須強制「逐字引用中文原文，絕對不接受英文 summary」（REFLEXES #16+31）
5. **Over-citing 偵測** — 一個 footnote 同時被綁住 5+ 個高風險 atom，但 source 只支持 1-2 個 → article 假裝有 source 但實際 leak（沈伯洋 [^7] 實戰）
...
| 檔案                                                     | 範圍                                                                                               |
| -------------------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| **本檔**                                                 | 事實查核 SSOT — Phase 1-6 + 8 atom 類 + 4 維度 source authority + Quick/Full Mode                  |
| [REWRITE-PIPELINE.md](REWRITE-PIPELINE.md)               | Stage 3.3 Quick Mode 觸發點 / Stage 3.5 Full Mode hallucination audit / Stage 3.6 STORY ATOM AUDIT |
| [RESEARCH.md](../editorial/RESEARCH.md)                  | 「怎麼搜資料」（從 0 蒐集）vs 本檔「怎麼驗證」（從 1 反查），方向相反                              |
| [EDITORIAL.md](../editorial/EDITORIAL.md)                | §挖引語制度 + §紀實 vs 煽情 是寫作層紀律，本檔是驗證層紀律                                         |
| [MAINTAINER-PIPELINE.md](MAINTAINER-PIPELINE.md)         | §Step 3.4 footnote source authority audit 觸發本檔 Quick Mode                                      |
| [PEER-INGESTION-PIPELINE.md](PEER-INGESTION-PIPELINE.md) | 策展 peer ingest 後對 atom 跑 audit（接 Phase 3-4）                                                |

**Quick Mode vs Full Mode**：

| Mode      | 預算       | WebFetch | 觸發                                            | 跑的 Phase              |
| --------- | ---------- | -------- | ----------------------------------------------- | ----------------------- |
| **Quick** | 30-60 min  | 5-15     | REWRITE Stage 3.3 / MAINTAINER Step 3.4 外部 PR | Phase 1, 3 採樣, 4 採樣 |
| **Full**  | 90-180 min | 25+      | A 級 ship 後 / 觀察者質疑 / 月度巡邏            | Phase 1-6 全跑          |
...
> **觸發新增 audit 的時機**：(a) 文章剛 ship 後（REWRITE-PIPELINE 內嵌跑 quick 版）(b) 讀者公開質疑（觀察者 callout / Issue / Threads 留言）(c) 月度巡邏（抽樣母體見 §月度巡邏抽樣母體：`lastHumanReview: false` × 出生最早 × 譯本最多加權，**不抽已走過 REWRITE 的 A/B 級**）(d) 外部 PR merge 前（接收層深度檢查）。
...
- 待 audit 文章路徑
- 觸發來源（REWRITE Step 3.3 / 觀察者 / Issue / 月度巡邏；v5.0 編號，舊版 Stage 3.5）
...
4. **保留完整 audit trail**：把 audit table 完整 append 到 `reports/research/YYYY-MM/{slug}.md` § audit section（REFLEXES #22 raw 永留）
...
| **月度巡邏 audit 5 篇**      | `general-purpose` × 5 平行 | 每篇獨立 audit；單一 prompt 可重用；母體依 §月度巡邏抽樣母體 抽，不抽 A 級                  |
...
每次 audit append 到 research 檔。即使後續發現舊 audit 結論錯，也保留。錯誤的 audit 結論本身是「為什麼後續改用本 pipeline」的證據鏈（REFLEXES #22 raw 永留 / MANIFESTO §時間是結構）。
...
| ---------------------------------------------------------- | -------------- | -------------- | --------------------------------------------------------- |
| **REWRITE-PIPELINE Step 3.3**（新文章 ship 前；v5.0 編號） | **Quick Mode** | 30-60 min      | 主 session 自跑                                           |
...
| **月度巡邏（依 §月度巡邏抽樣母體 抽 5 篇未審初稿）**       | Full Mode × N  | 5 × 60-180 min | 平行 spawn                                                |

### Quick Mode（REWRITE Step 3.3 內嵌；v5.0 編號，舊版 Stage 3.5）
...
### 月度巡邏抽樣母體（v2.1，2026-09-18）

巡邏抽的層必須是病在的層。A/B 級是已經走過 REWRITE Stage 3.3 幻覺審計的文章，是全站最不需要巡邏的那批；幻覺密度最高的是三月出生、從未人工審核、腳註只有幾條泛連結的 AI 初稿，而這批多數已被巴別塔忠實投射到十語以上。翻譯線的閘門全量「譯文對原文忠不忠實」，原文層的錯會一字不差放大到每一個語言，所以**譯本數是放大係數**：同樣一個幻覺，十語譯本的比零譯本的先查。
...
**修改 SOP 時的紀律**：任何 atom 類別、hallucination pattern、drift mode、checklist 條目的新增/修改 → **只改本檔**。REWRITE 自動繼承新版本（因為它指向本檔）。違反此紀律 = 認知層 SSOT 漂移（REFLEXES #17 第 N 次驗證）。
...
_v2.0 | 2026-05-11 cranky-newton — Spine restoration 對齊 REWRITE v5.0 + MAINTAINER v2.0：頂部加 ASCII spine（Phase 1-6 + Article Tier + Quick/Full Mode box-frame）+ Hard Gate Inventory 集中 table（15 gates）+ Top 5 最常忘 step + 跨檔案職責分工 standalone table + Quick/Full Mode 對照表。觸發：[reports/pipelines-audit-2026-05-11.md](../../reports/pipelines-audit-2026-05-11.md) Tier A.3 audit。Phase 1-6 prose body 不動（已健康，refactor 主要在頂部加導航）。_
_作者：Taiwan.md（θ）_
