...
last_session: '2026-09-05-rewrite-single-file-generator（哲宇拍板 v9 為 SSOT；REWRITE-PIPELINE 兩列改寫成 v9 SSOT／單檔生成版／v8.0 archive 三者關係）'
sister_docs:
...
| [REWRITE-PIPELINE.md](REWRITE-PIPELINE.md)                                   | 手動觸發                | 寫文章 / 重寫文章 SOP（v9 薄索引 ＋ 十一個 `REWRITE-STAGE-*.md` contract，**SSOT**；2026-09-05 哲宇拍板）                                                                                                                                                                                                                                                                        |
| [REWRITE-PIPELINE-單檔案型完整流程.md](REWRITE-PIPELINE-單檔案型完整流程.md) | 手動閱讀（不手改）      | 單檔閱讀版，由 `python3 scripts/tools/build-rewrite-single-file.py` 依上一列的派發表順序重組全部 stage contract 自動生成；改 v9 來源後重跑產生器即可，pre-commit 掛新鮮度 `--check`。拆檔前最後一版手寫單檔（v8.0，2026-07-15，逐字凍結）存檔於 [archive/REWRITE-PIPELINE-v8.0-single-file-2026-07-15.md](archive/REWRITE-PIPELINE-v8.0-single-file-2026-07-15.md)，僅供歷史對照 |
