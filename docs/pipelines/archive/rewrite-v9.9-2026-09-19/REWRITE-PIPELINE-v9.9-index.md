...
> ⏸️ **留校察看（2026-09-19 哲宇拍板）**：本檔與十個 `REWRITE-STAGE-*.md` contract（v9 多檔型）**暫停使用**。現行 canonical 是單檔型 [REWRITE-PIPELINE-SINGLE.md](REWRITE-PIPELINE-SINGLE.md)（v6.7 續行 v6.8）。原因：9/18 三篇走 v9 互動式六站＋十五位冷讀者＋三輪 release 全綠上線，哲宇讀後「文謅謅、沒人味、像堆砌不像有主見的分析、過度強調數字」；隔天同三篇用單檔型重做，閱讀感明顯好轉。v9 的 contract 今天同步修了論點規則（STAGE-0 v8.0）、找人 lane 與 ~100 次配額（STAGE-1A v9.10）、opening-readability（STAGE-4 v9.10）、冷讀站移除（REWRITE-GUIDE v1.1），留著等下一次對照實驗；在那之前 `/twmd-rewrite` 只讀單檔。
...
| **研究報告 SSOT health** 🔬             | Stage 1 終 | **所有 depth**                                       | `research-report-health.py --tier=depth`（distinct≥25 / en≠0 / 一手≠0 / 搜尋日誌 / 信度三層 / **v2 §8 有效密度 ≥120 + ephemeral pointer=0** / **v4 合成層過程噪音 ≤3——Findings 寫世界不寫任務，[Step 1.7.5 整合與清理六判準](REWRITE-STAGE-1A-RESEARCH.md)**；v2.1 疑慮通知層：每條 fail/warn 附為什麼+思考方向）                                                                     | **不進 Stage 2**           |
...
> **v9.6 搬遷（2026-08-06 pipeline-shell-lint 瘦身）**：Token 預算分配／Cron 鐵律／合法 defer
> signal 六條／選文與 commit 指令／Cron 狀態／Routine 飛輪整合（v6.1 full-cycle）verbatim 搬至
> [REWRITE-STAGE-0-VIEWPOINT.md §Cron 模式 + Routine 飛輪](REWRITE-STAGE-0-VIEWPOINT.md#cron-模式--routine-飛輪2026-08-06-從-rewrite-pipelinemd-主檔-verbatim-搬入pipeline-shell-lint-瘦身)
> （掛在 Stage 0 入口 contract，defer signal 判斷發生在進 Stage 0 之前）。
