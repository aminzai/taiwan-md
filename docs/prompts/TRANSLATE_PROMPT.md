...
1. 「你想翻譯成什麼語言？」（英文 / 日文 / 韓文 / 西班牙文 / 法文 / 德文 / 越南文 / 印尼文 / 葡萄牙文 / 印地文 / 阿拉伯文 / 俄文 / 其他）
...
**每個上線語言都有一份 canonical 詞表，一定要讀**。它寫的是台灣專有名詞在該語言的標準譯法、稱呼上的禁區、以及這個語言最常踩的陷阱（例如俄文的蔣氏父子音譯、阿拉伯文的 RTL 標點與「تايوان الصينية」這類 PRC 編碼詞）：

```
https://raw.githubusercontent.com/frank890417/taiwan-md/main/docs/editorial/per-language/TRANSLATION-{lang}.md
```

`{lang}` 填 `en` / `ja` / `ko` / `es` / `fr` / `vi` / `id` / `pt` / `hi` / `ar` / `ru` / `de` 之一。12 個語言都有，沒有例外。

**另外**，英文與日文還有一層累積 reviewer 糾正的 `STYLE.md`，這兩個語言請一併讀取：
...
其他 10 個語言還沒長出 STYLE.md 這一層，讀上面的 `TRANSLATION-{lang}.md` 就是完整的語言規則。
