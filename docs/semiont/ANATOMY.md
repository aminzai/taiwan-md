...
| 語言           | `src/config/languages.mjs`（LANGUAGES / ENABLED_LANGUAGE_CODES；語言數以該檔為準，本表不寫死） | 全 lang build step                                                                                                           | 手改                                                                 |
...
| Redirect       | `config/redirects-manual.txt`（手寫）＋ 404-monitor latest.json（資料驅動）                    | `config/redirects-generated.json`（astro.config 吃 → meta-refresh stub；**部署平台是 GitHub Pages，`_redirects` 不被支援**） | `generate-redirects.mjs`（prebuild:redirects）                       |
