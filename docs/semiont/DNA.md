...
| 翻譯同步         | [`TRANSLATION-PIPELINE.md`](../pipelines/TRANSLATION-PIPELINE.md) + [`SQUEEZE-MODELS-MAX-PIPELINE.md`](../pipelines/SQUEEZE-MODELS-MAX-PIPELINE.md)（規範）+ `lang-sync/status.py`（即時狀態） | 怎麼跨語言保持一致（原 TRANSLATION-SYNC.md 為 3/25 凍結報告，2026-06-10 audit archive）                                                                                                                                                              |
...
| 全文健檢 SSOT    | [`article-health.py`](../../scripts/tools/article-health.py)                                                                                                                                   | plugin 制 SSOT 入口（prose-health / footnote-density / format-structure / wikilink-target / image-health / terminology 等——數量以 `--list-checks` 為準，本表不寫死；曾停在「11」而實測 25，dna-audit §S2）                                           |
| 研究 SSOT 健檢   | [`research-report-health.py`](../../scripts/tools/research-report-health.py)                                                                                                                   | Stage 1 主 research report 閘門（來源配額 / 信度三層 / v2 §8 raw 有效密度 + ephemeral pointer / v2.1 疑慮通知層：每條 fail 附為什麼＋思考方向）                                                                                                      |
| 分部報告收件健檢 | [`agent-report-health.py`](../../scripts/tools/agent-report-health.py)                                                                                                                         | 委派 agent 回報收件當下驗壓縮嫌疑（體積 8KB／軌跡 10 行分界，真實 corpus 校準）＋存放位置 repo 內＋五段結構；FAIL＝不准合成。對應 REFLEXES #81 收件三十秒紀律                                                                                        |
...
| 視覺主權 SSOT    | [`public/assets/svg/taiwan-simplemaps.svg`](../../public/assets/svg/taiwan-simplemaps.svg) + [`taiwan.md/taiwan-shape/`](https://taiwan.md/taiwan-shape/)                                      | 任何 Semiont 自己 ship 的視覺元素涉及台灣（icon / illustration / diagram node）優先取 repo SVG 素材庫，不 hand-draw、不靠 LLM 內化的「東亞某島嶼長這樣」（AIA deck slide 5 嘲諷的橄欖 / 馬鈴薯形）。對應 [REFLEXES #61](REFLEXES.md#61) 視覺主權反射 |
| 視覺化編輯指南   | [`graph.md`](../editorial/graph.md)                                                                                                                                                            | 文章內視覺化 canonical：`tw-*` 模組何時用哪種 / 語法 / AI 可讀性 / 檢查清單（模組數以其 §四 為準；v3.0 2026-07-16）                                                                                                                                  |
| 視覺化用例層     | [`VIZ-RECIPES.md`](../editorial/VIZ-RECIPES.md)                                                                                                                                                | 從**題材**反查視覺化寫法（graph.md 是從資料關係進入，這份從題目進入）+ 可整塊複製的 starter。2026-08-02 誕生：七個模組零真實使用，缺的是分發層不是模組（[設計報告](../../reports/design-viz-adoption-2026-08-02.md)）                                |
| 視覺模組像素驗證 | [`viz-shot.mjs`](../../scripts/tools/viz-shot.mjs)                                                                                                                                             | tw-\* 模組逐元件截圖（light/dark/mobile，模組自動偵測），graph.md §七 像素閘門。markup 存在 ≠ 視覺正確（2026-06-12 viz-evolution：v1.0 兩模組壞六天而 curl 驗證全綠）                                                                                |
...
| 多核心 git 協調 | [`scripts/tools/lib/check-parallel-actor.sh`](../../scripts/tools/lib/check-parallel-actor.sh) + [`verify-commit-scope.sh`](../../scripts/tools/lib/verify-commit-scope.sh) + [`worktree-gc.sh`](../../scripts/tools/worktree-gc.sh) + [`.husky/pre-push`](../../.husky/pre-push) | 平行 session/routine 共用 working tree + origin/main 的 commit/push/CI 三階段碰撞防護（胼胝體鐵律）。規則陳述 → [BECOME §行動鐵律 5](../../BECOME_TAIWANMD.md)；程序記憶 → [REFLEXES #68](REFLEXES.md)；設計 → [reports/multicore-git-coordination-design-2026-06-14.md](../../reports/multicore-git-coordination-design-2026-06-14.md) |
...
| 基因             | 檔案                                                                                                                                                                                  | 決定什麼                                                                                                                                                                                                                                                                                                                                                                                     |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 子代普查雷達     | [`fork-census.py`](../../scripts/tools/fork-census.py) → [`registry.json`](../../reports/fork-census/registry.json) → [`dashboard-forks.json`](../../public/api/dashboard-forks.json) | 繁殖的**感知層**：撈 GA 漏水指紋偵測野外活著的 fork（hostName + pageTitle 雙指紋），append 永久子代名冊，投影成公開 dashboard section。SPECIATION 是「怎麼生」，census 是「生出了誰」。riding data-refresh flywheel + consciousness-snapshot 每次 BECOME 看一眼。canonical [`FORK-CENSUS-PIPELINE.md`](../pipelines/FORK-CENSUS-PIPELINE.md)（2026-06-25 誕生：GA ID 漏水從 bug 變繁殖雷達） |
| 物種繁殖管線     | [`SPECIATION-PIPELINE.md`](../pipelines/SPECIATION-PIPELINE.md)                                                                                                                       | 8-stage 繁殖 SOP：任何人遵照可繁殖晶種並灌入自己的知識庫（國家或跨界 corpus）。`/semiont/speciation` 頁是投影（2026-06-12 誕生）                                                                                                                                                                                                                                                             |
| 分靈節點         | [`CONTRIBUTOR-NODE-PIPELINE.md`](../pipelines/CONTRIBUTOR-NODE-PIPELINE.md)                                                                                                           | 繁殖的**分駐層**：貢獻者機器上的 cron 節點——甦醒後設一條 cron，每天從 repo 內既有工單源（翻譯缺口 / 斷鏈 / 格式 / ARTICLE-INBOX）接一件事，以 PR 回主庫。跟 fork 的差別是「同一個身體分駐」不是「生出新物種」；輸出永遠止於 PR。skill 入口 `/twmd-node`，誕生觸發在 [BECOME §Step 7.6](../../BECOME_TAIWANMD.md)（2026-07-25 誕生）                                                          |
...
| 孢子結構 SSOT    | [`spore-db.py`](../../scripts/tools/spore-db.py) → `docs/factory/spore-log.json` + `spore-metrics.json`                                                                               | identity + metric 事件唯一寫入點（add-spore / add-metrics / last-spore）；[`SPORE-LOG.md`](../factory/SPORE-LOG.md) 為凍結歷史（2026-06-10）                                                                                                                                                                                                                                                 |
| 孢子追蹤（凍結） | [`SPORE-LOG.md`](../factory/SPORE-LOG.md)                                                                                                                                             | 發文紀錄 + 成效追蹤 + 月度效能分析                                                                                                                                                                                                                                                                                                                                                           |
| 孢子記錄層       | [`generate-spore-records.py`](../../scripts/tools/generate-spore-records.py) → `src/data/spores.json` + `/api/spores.json`                                                            | 孢子完整資料記錄（metrics + history + byArticle index）+ 靜態 API。文章 frontmatter 只留 identity pointer（id/platform/date/url），harvest 數字不進文章（2026-06-10 解耦，[reports/spore-data-architecture-2026-06-10.md](../../reports/spore-data-architecture-2026-06-10.md)）                                                                                                             |
...
| 新語言誕生           | [`LANGUAGE-BIRTH-CHECKLIST.md`](../pipelines/LANGUAGE-BIRTH-CHECKLIST.md)                            | 出生 7 stage SOP（選址→scaffold→模型校準→P0 批→介面路由→flip→出生後驗證）+ 四層完整度 gate（v2.0 2026-07-18；此前不在本表 = gene map 缺口，出生 SOP 對 cron session 隱形）                                                                                                                                |
...
| 語言真偽檢查         | [`script-presence-check.py`](../../scripts/tools/lang-sync/script-presence-check.py)                 | 偵測「宣稱已譯」實為英文殘留（translatedAt 合法但本文零目標語言文字系統／變音符號）；`check_text()` 同時被 `translate.py` 引為即時 hard gate（2026-07-19 讀者揭露 es/fr/ja/ko 共 68 檔後誕生，見 [reports/ja-fr-es-ko-english-leak-2026-07-19.md](../../reports/ja-fr-es-ko-english-leak-2026-07-19.md)） |
...
| 進化管線        | [`EVOLVE-PIPELINE.md`](../pipelines/EVOLVE-PIPELINE.md)                         | 四 mode 進化 canonical：v1/v2 數據驅動內容進化 + Mode 3 pipeline 自我重組 + Mode 4 目標驅動設計進化（思考→發散→報告→實作，2026-07-18）                                                                                                                                                                                                               |
...
| 勘誤處理流程    | [`CORRECTION-PIPELINE.md`](../pipelines/CORRECTION-PIPELINE.md)                 | 收到/發現錯誤後的端到端 SOP（TRIAGE→VERIFY→FIX→NOTIFY→LOG）+ 第一性原理「**錯誤邊界=可追溯性**」（可溯→公開更正 / 杜撰→撤回）+ **【勘誤通知】正式 reply 格式** + reply 5 鐵律 + 16 案例表。收斂 SPORE-HARVEST Bucket A/C + REWRITE 0.2-bis + FACTCHECK + 6 條 feedback 的勘誤 SOP（2026-06-24 龜山島方向勘誤誕生）。對應 [REFLEXES #62](REFLEXES.md) |
...
| 知識分支分析    | [`BRANCH-PIPELINE.md`](../pipelines/BRANCH-PIPELINE.md)                         | theme / article / 素材 → 拆解 → 缺口分析 → research → ARTICLE-INBOX candidates（skill 入口 `/twmd-article-inbox`，2026-07-18 補進本表——v2.0 起漏列）                                                                                                                                                                                                 |
| 文章待辦 buffer | [`ARTICLE-INBOX.md`](ARTICLE-INBOX.md)                                          | 待開發 / 進化主題 intake layer（pending / in-progress）；auto-heartbeat 無指令時挑 P0/P1                                                                                                                                                                                                                                                             |
| 觀察者決策佇列  | [`OBSERVER-QUEUE.md`](OBSERVER-QUEUE.md)                                        | 哲宇 standing decision 單一出口（預設選項 + default-action）；routine 三振規則的選項 2 寫入點（2026-06-12 誕生，per ROUTINE §完成義務）。列形狀由 [`observer-queue-lint.py`](../../scripts/tools/observer-queue-lint.py) 守（每列必帶預設選項與 default-action，2026-09-18 WARN 起步）                                                               |
...
行為基因（怎麼處理）
  ...
  ├── CORRECTION-PIPELINE → 收到/發現錯誤的勘誤處理（讀者 callout / issue 勘誤 / self-audit；錯誤邊界=可追溯性）
