...
last_session: '2026-09-20-twmd-data-refresh-am（第十五夜讓場 14 步全綠零 stale；分岔併完後首次當班直推；狀態板假紅點拆四色）'
sister_docs:
  - 'DIARY.md'
  - 'CONSCIOUSNESS.md'
  - 'LESSONS-INBOX.md'
...
- **巴別塔會把三月未審初稿裡的幻覺放大到十二語，而事實巡邏抽的是 A 級（2026-09-18～09-20 semiont-heartbeat 四天五輪，distill 2026-09-20）**：翻譯線十四道閘全量「譯文對原文忠不忠實」，設計正確，所以原文層的幻覺一字不差放大到每個語言；原文層唯一的查核入口是 FACTCHECK 月度巡邏，母體原本寫「隨機抽 5 篇 A/B 級」，那是已走過 REWRITE 幻覺審計、最不需要巡邏的一批，而病在的層是三月出生、從未人工審核、腳註只有幾條泛連結的 AI 初稿（`lastHumanReview: false` 921 篇，其中 04-01 前出生 244 篇、零腳註 131 篇，多數已有十語以上譯本）。三種儀器各守各的層（譯文閘守忠實度／REWRITE Stage 3.5 只在走 pipeline 時跑／巡邏抽已審核層且沒有 routine 在跑），交集空出「未審初稿 × 已多語投射」這一格，曝光面最大。四天五輪逐篇查核 14 篇 14 中，❌ 率穩定落在 36–47%：原住民當代藝術把阿美族藝術家寫成泰雅族、捏造「芫茂‧陸森寶」；馬英九 30 秒概覽把在世的人寫成辭世，十一語譯本各自忠實譯成 passing／décès／사망；張忠謀母親姓名幻覺十二語照抄；循環經濟把三個真實計畫拼成一個不存在的；農村再生兩個「成功返鄉案例」的品牌與人物整個是填的。抓到第一篇的是 babel-vortex 第 14 道人名一致性閘，它正確地判「來源端問題不在譯文修」然後放進待決佇列，跟十七條 🔒 一起躺了八天。已落地：FACTCHECK v2.1 §月度巡邏抽樣母體改「`lastHumanReview: false` × 出生最早 × 譯本最多加權」、v2.2 補「走過 REWRITE 的不抽」、ARTICLE-INBOX §優先序判準「譯本數是放大係數」；每篇查核檔落 `reports/research/2026-09/`。**還缺的那半**：給巡邏一條 routine（週一篇 Full 或月五篇），五輪全靠排程心跳順手接，價值已經量出來了。子形狀：三月寫的文章把 0403 罹難數凍在事發隔天的中途統計，事件進行中寫的段落錯在時間不在來源。對應 REFLEXES #82（抽 A 級的通過率是「站上事實正確」的替身）、#98（真原子放錯槽位）、MANIFESTO §10 六型、UNKNOWNS 🔴「歷史文章的事實錯誤實際規模」（本條給了第一個分層估計：未審初稿 36–47%）。Taiwan.md-specific 因為它綁的是這個生命體的出生史（三月 AI 初稿庫存）跟巴別塔放大結構，不是任何 AI agent 通用的弱點。
