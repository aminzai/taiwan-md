...
- **headless 機器遷移驗證要選「真的那層」的尺，憑證一律檔案層儲存（2026-07-24 migration-mouhouse，distill 2026-07-26）**：把 routine 飛輪工作搬到一台 headless 機器時，語法掃描（`ast.parse` 全 3.9 通過）不等於 runtime 相容（`npm run build` 才炸出 PEP 604 `str | None` annotation 需要 3.10+）——build 煙霧測試才是真尺。同批第二課：`gh` 在 SSH session 授權成功但 token 寫進鎖住的 GUI keychain，`git push` 拿不到；自動化機要 `gh auth login --insecure-storage` 把憑證落檔案層。未來任何機器遷移／fleet 節點 onboard checklist 應含這兩項：build 煙霧測試必跑 + 憑證一律檔案層儲存。 **第二次驗證（2026-09-19 twmd-babel-nightly）**：同一台機器上把 babel keepalive 的 wrapper 從 /tmp 搬進 repo 重新 `launchctl submit`，第一輪就 crash-loop——launchd 不讀 shell profile，`python3` 是 Apple 3.9，status.py 的 `str | None` 當場炸；前一版能跑只因 submit 時繼承了帶 venv 的 PATH。checklist 補第三項：任何由 supervisor（launchd／cron）啟動的入口，直譯器路徑明寫進檔案，不靠環境。
...
- **巴別塔會把三月未審初稿裡的幻覺放大到十二語，而事實巡邏抽的是 A 級（2026-09-18～09-20 semiont-heartbeat 四天五輪，distill 2026-09-20）**：翻譯線十四道閘全量「譯文對原文忠不忠實」，設計正確，所以原文層的幻覺一字不差放大到每個語言；原文層唯一的查核入口是 FACTCHECK 月度巡邏，母體原本寫「隨機抽 5 篇 A/B 級」，那是已走過 REWRITE 幻覺審計、最不需要巡邏的一批，而病在的層是三月出生、從未人工審核、腳註只有幾條泛連結的 AI 初稿（`lastHumanReview: false` 921 篇，其中 04-01 前出生 244 篇、零腳註 131 篇，多數已有十語以上譯本）。三種儀器各守各的層（譯文閘守忠實度／REWRITE Stage 3.5 只在走 pipeline 時跑／巡邏抽已審核層且沒有 routine 在跑），交集空出「未審初稿 × 已多語投射」這一格，曝光面最大。四天五輪逐篇查核 14 篇 14 中，❌ 率穩定落在 36–47%（09-20 下午第六輪三篇 17 中 17：農業科技篇 36%，兩篇地理姊妹稿 23–24%——教科書級地理數字大多對，錯在五十年前的舊值與地名放錯山脈，農業政策段的目標數字則幾乎全是填的）：原住民當代藝術把阿美族藝術家寫成泰雅族、捏造「芫茂‧陸森寶」；馬英九 30 秒概覽把在世的人寫成辭世，十一語譯本各自忠實譯成 passing／décès／사망；張忠謀母親姓名幻覺十二語照抄；循環經濟把三個真實計畫拼成一個不存在的；農村再生兩個「成功返鄉案例」的品牌與人物整個是填的。抓到第一篇的是 babel-vortex 第 14 道人名一致性閘，它正確地判「來源端問題不在譯文修」然後放進待決佇列，跟十七條 🔒 一起躺了八天。已落地：FACTCHECK v2.1 §月度巡邏抽樣母體改「`lastHumanReview: false` × 出生最早 × 譯本最多加權」、v2.2 補「走過 REWRITE 的不抽」、ARTICLE-INBOX §優先序判準「譯本數是放大係數」；每篇查核檔落 `reports/research/2026-09/`。**還缺的那半**：給巡邏一條 routine（週一篇 Full 或月五篇），五輪全靠排程心跳順手接，價值已經量出來了。子形狀：三月寫的文章把 0403 罹難數凍在事發隔天的中途統計，事件進行中寫的段落錯在時間不在來源。對應 REFLEXES #82（抽 A 級的通過率是「站上事實正確」的替身）、#98（真原子放錯槽位）、MANIFESTO §10 六型、UNKNOWNS 🔴「歷史文章的事實錯誤實際規模」（本條給了第一個分層估計：未審初稿 36–47%）。Taiwan.md-specific 因為它綁的是這個生命體的出生史（三月 AI 初稿庫存）跟巴別塔放大結構，不是任何 AI agent 通用的弱點。
...
| 2026-09-23 | 051718-twmd-embeddings-nightly | 13 語 13,795 向量 0 fail（+103 集中 de／id），46 分鐘零分岔；殼層修補交給 routine-sync 被結構性退回，本班自己改 git SSOT 收掉四處寫死 | 交接前要驗收件席位有沒有權限；canonical 修好的寫死在殼層抄本活了兩個月，讀的人自動補正所以沒摩擦感 | [→](memory/2026-09-23-051718-twmd-embeddings-nightly.md) |
| 2026-09-23 | 061513-twmd-data-refresh-am | 第十八夜讓場 14 步全綠零 stale；404 探路檔名整族收進掃描器，unknown 52.8% → 46.2%；routine 面板遲一天的成因是 Step 6.6 排在 live-state 落檔之前 | 命名一個現象會讓它看起來已被處理過；同一個日期的 404 總數取決於哪一刻查它 | [→](memory/2026-09-23-061513-twmd-data-refresh-am.md) |
| 2026-09-23 | 064834-twmd-spore-harvest-am | Chrome 無視窗第二次，`open -a` 八秒自修不再交人；兩動態頁 0 新留言 0 桶 0 ship 合法 no-op；已決的 manual-by-decision 被 26 班交接寫成「未拍板」 | 探針回空先問為什麼連不上再決定交不交人；有一班更正過，更正只住在它自己的檔案裡 | [→](memory/2026-09-23-064834-twmd-spore-harvest-am.md) |
...
| 2026-09-23 | 084836-twmd-maintainer-am | aminzai 三篇譯文收下；死連結比例綠燈底下追出一個十二語 60 條的家族，根因是母稿延伸閱讀寫成裸檔名；順手把 09-07 留下的「斜體圖說帶底線網址」未解項量完＝全庫三篇 | 角括號救不了網址，會改壞它的是外面那層斜體；比對網址集合看不見壞掉的那一份，要比出現次數 | [→](memory/2026-09-23-084836-twmd-maintainer-am.md) |
