...
last_session: '2026-09-17-semiont-heartbeat（fncard 逾期補判 + vi/pt 到期日當天判：兩條部分命中、一條命中）'
sister_docs:
  - 'MANIFESTO.md'
  - 'DIARY.md'
  - 'CONSCIOUSNESS.md'
...
| ~~有多少讀者從外部舊連結進來打到 404~~ → **可回答了（2026-07-17）**：page_404 事件（含 referrer 維度）死了三個月後復活，D+7 起 GA4 可拆內部斷鏈 vs 外部舊連結；CF 側大宗已證實是自吐 hreflang 死鏈（EXP-2026-07-17-G）                                    | 2026-04-04 |
...
### ~~EXP-2026-08-28-fncard | 腳註來源卡有沒有真的把讀者送到來源~~ 🟡 **(b) 命中 / (a) 部分命中（2026-09-17 heartbeat 補判，逾期 6 天）**

> **2026-09-17 判定**：到期日 2026-09-11 過了 6 天，黃燈掛在 dashboard-alerts 上沒人判（09-11 到 09-16 之間本機跟營運機各自分岔，週日體檢的產出在救援分支上）。當場跑 EXP 自帶的 `footnote-card-adoption.py --start 14daysAgo`（窗口 09-03〜09-17，比原設計的 D+14 窗口晚六天，樣本更大不更小）。
>
> **(b) 命中**：展開之後真的點開來源的比例 **18.9%**（250 次點開 ÷ 1,323 次展開；146 人），門檻 15%。展開的觸發方式 hover 83.6%／click 16.3%，代表大多數展開是游標滑過去的順手動作，而順手動作裡仍有近兩成走到來源。
>
> **(a) 部分命中**：展開卡片的不重複使用者 676 人，全站 14 天不重複使用者 43,299 人（GA4 `totalUsers`），佔 **1.56%**——高於 1% 的「卡片沒被發現」反駁線，低於 3% 的命中線。對照組：同一窗口滑到文末腳註區的舊路徑是 6,850 次（12.0% of PV），新路徑 1,323 次展開是舊路徑的五分之一。
>
> **判讀**：距離確實是阻力的一部分（b 證明搬到游標旁的來源會被點），但記號 `[n]` 的可供性還不足以讓多數讀者發現它會展開（a 未達 3%）。依原設計「介於門檻之間：部分命中，續觀察一個窗口」——**不重開 EXP、不動記號設計**，下一次體檢用同一支指令再讀一次；若 (a) 兩個窗口都停在 1〜2%，再考慮記號本身的視覺提示。同一套機制服務 OBSERVER-QUEUE #39 正文內鏈的判斷，等 (a) 第二窗口。

> ~~機械到期檢查：due_date: 2026-09-11 | EXP-2026-08-28-fncard~~（已判定，marker 除役）

- **預測**：腳註來源卡（2026-08-28 上線，`e54a4a055`）部署 14 天後，(a) **展開卡片的不重複使用者 ≥ 全站不重複使用者的 3%**；(b) **展開之後真的點開來源的比例 ≥ 15%**（`outbound_click(section=footnote_card)` ÷ `footnote_card_open`）
- **根據**：上線前的成本量測是「腳註躺在 3-25 個螢幕之外，只有 12.3% 的頁面瀏覽抵達文末那一區」。如果距離真的是主要阻力，把來源搬到游標旁應該讓一部分本來不會查證的讀者查證。**這個功能沒有更早的基準可比**——站上 17,113 條來源連結在此之前一個埋點都沒有，所以歷史的「0 次點擊」是 0 筆資料。上線後第一小時的自然量（非我自己的測試流量）是 19 次展開 / 7 人 / 8 次點開，落在門檻之上，但樣本太小不算數。設計與量測全文：[reports/design-footnote-source-cards-2026-08-28.md](../../reports/design-footnote-source-cards-2026-08-28.md)
- **驗證指令**：`python3 scripts/tools/footnote-card-adoption.py --start 14daysAgo`
- **反駁條件**：
  - (a) ≥ 3% 且 (b) ≥ 15%：**命中**——距離確實是主要阻力，這條路值得再往前推（同一套機制可服務 OBSERVER-QUEUE #39 的正文內鏈）
  - (b) < 5%：**反駁**——讀者要的是「這句話有沒有出處」這個確認本身，不是那個出處。卡片變成純確認裝置，那就該把力氣改花在讓卡片自己說清楚（來源類型分層、發布年份），而不是想辦法把人送出站
  - (a) < 1%：**反駁**——卡片沒被發現。`[n]` 的視覺提示不足以讓讀者知道它現在會展開東西，要回頭處理記號本身的可供性
  - 介於門檻之間：**部分命中**，續觀察一個窗口
- **量測污染註記**：上線當天 session 自己用 Playwright 對正式站跑過驗證，貢獻約 2-3 次 `footnote_card_open` 與 1 次 `content_click`。判定時窗口是 14 天，這幾筆可忽略；但如果之後有人再對正式站跑自動化驗證，要在這裡記一筆

### ~~EXP-2026-07-25-alias | 英文別名側門的需求組成驗證~~ ✅ **(a) 命中 / (b) 不可判（2026-08-30 週體檢補判，逾期 6 天）**

> **2026-08-30 判定**：到期日 2026-08-24 過了 6 天，黃燈掛著沒人動手（上一輪體檢的交接清單寫了「用它自己的指令判，不提前也不延後」，時間到了就是今天）。當場跑 EXP 自帶的 `monitor-404.py --days 7`。
>
> **(a) 命中，而且低於門檻一個數量級**。cross-lang-slug 家族過去七天逐日 4 / 1 / 0 / 1 / 0 / 1 / 1 / 2（8/21–8/28），日均 **1.4**，對基線 538/日 是 **−99.7%**，門檻是 ≤50。「這個家族的來源是拿 en 路徑去掉 `/en/`」這個單一機制判斷成立，827 條別名近乎全覆蓋，沒有第二種未分類的大宗來源。
>
> **(b) 不可判，而且是被 (a) 的成功消解掉的**。七天合計只剩 10 次請求（bot 6 / browser 4），形式上 40% 過 30% 門檻，但 n=10 撐不起任何決策——基線是 235/538。真正的結構變化是：別名層有效到讓「走別名進站」幾乎不再產生 404，於是這個 404 家族裡的人機比，**已經不再代表「有多少真人在複製中文網址」**。它從一個需求訊號變成了一個殘餘噪音。
>
> **對那個待決的影響**：「分享按鈕要不要改成英文別名」不能再靠這條 404 家族回答，需要另一個量測面（例如分享按鈕自己的 `outbound_click` 分層，或站上複製事件）。這一點寫進報告給哲宇，不自行改判。**教訓形狀**：一個實驗的成功條件達成得太徹底時，會把它自己的第二個問題一起關掉——(b) 從一開始就寄生在 (a) 的失敗上。設計雙條件實驗時要問「如果第一條完美命中，第二條還量得到東西嗎」。

> ~~機械到期檢查：due_date: 2026-08-24 | EXP-2026-07-25-alias~~（已判定，marker 除役）

- **預測**：827 條英文別名（`04a8657de`，2026-07-25 上線）部署 30 天後，(a) `monitor-404.py` 的 **cross-lang-slug 家族從基線 538/日降到 ≤ 50/日**；(b) 走別名進站的請求裡，**真人瀏覽器佔比 ≥ 30%**（基線該家族 235/538 = 43.7% 是瀏覽器）
- **根據**：上線前抽樣 10 條猜測 slug 對照 `knowledge/en/` 檔名 10/10 精確命中，代表這個家族的來源是「拿 en 路徑去掉 `/en/`」單一機制，別名應該近乎全覆蓋。殘餘量若仍高，表示還有第二種未分類的來源。完整量測與設計：[memory/2026-07-25-192633-article-alias.md](memory/2026-07-25-192633-article-alias.md)
- **驗證指令**：`python3 scripts/tools/monitor-404.py --days 7`（看 families 的 cross-lang-slug count 與 bot/browser 拆分）
- **反駁條件**：
  - cross-lang-slug ≤ 50/日：**命中**（單一機制判斷正確，別名層有效）
  - 仍 > 200/日：**反駁**——「都是拿 en 路徑去掉前綴」的判斷錯了，另有大宗來源；回去讀該家族的 top_paths 重新分類
  - 介於 50-200：**部分命中**（機制對、爬蟲快取衰減比預期慢，多等兩週再判）
- **這個實驗同時餵一個待決**：(b) 的真人佔比是「分享按鈕要不要改成英文別名」的決策輸入。真人顯著 → 複製痛真實存在，值得談；幾乎全是機器 → 這層本來就是為 AI 讀者鋪的，中文網址維持唯一正門（per [MANIFESTO §自稱](MANIFESTO.md)）。拍板權在哲宇

### ~~EXP-2026-07-18-vi | 越南文出生後需求轉換驗證~~ 🟡 **部分命中（2026-09-17 heartbeat 到期日當天判）**

> **2026-09-17 判定**：vi 支系 07-19 出生，D+30〜D+60 窗口取 08-17〜09-16。`sc-query.py --filter country=vnm`：**clicks 515 / impressions 46,760 / CTR 1.1% / 平均排名 8.1**。基線（2026-06 整月）：clicks 30 / impressions 7,254 / CTR 0.4%。
>
> **CTR 1.1% 落在 1.0〜2.0% 的「部分命中」帶**：需求是真的（點擊從月 30 → 515，17 倍），但轉換弱於預測的 2.0%。**預測沒料到的是分母**：曝光從 7,254 → 46,760（6.4 倍），Google 把 vi 內容推給的越南搜尋者遠多於出生前那批「有曝光無語言」的缺口，CTR 被新曝光稀釋。這跟反駁條件「7,994 曝光是品牌詞雜訊或 bot」相反——曝光是真人查詢在長，只是 vi 頁面在結果頁的吸引力（title／description）還沒跟上。
>
> **下一步**：依原設計「需求真實但轉換弱於預期，可能是 Hub 策展或 metadata 問題」——下次 evolve 語言層拿 vnm 的 top query 對照 vi 頁面 title，跟 en metadata 專項（roadmap P0-1）同一種修法。不重開 EXP。

> ~~機械到期檢查：due_date: 2026-09-17 | EXP-2026-07-18-vi~~（已判定，marker 除役）

- **預測**：vi 支系 P0 批（~65 篇＋13 Hub）上線後 60 天，**vnm 市場 SC CTR 從基線 0.5% → ≥ 2.0%**（Google 已在越南給曝光 7,994/月但無語言承接的缺口被接住）
- **根據**：選址時 vnm 是未服務語言中 SC 曝光最高（[evolve-2026-07-18-language-branches.md](../../reports/evolve-2026-07-18-language-branches.md)），CTR 0.5% 純粹是「有曝光無對應語言」。若判斷正確，vi 內容一上線該市場 CTR 應向全站均值（1.7%）收斂甚至超越（母語承接 + 新住民在地需求）
- **驗證指令**：`python3 scripts/tools/sc-query.py --dims country --filter "country=vnm" --start <D+30> --end <D+60>`
- **反駁條件**：
  - D+60 vnm CTR ≥ 2.0%：**命中**（需求判斷正確，vi 是島內導向支系的第一個成功案例）
  - D+60 仍 < 1.0%：**反駁**——7,994 曝光可能是品牌詞雜訊或 bot，不是真人需求；回頭查 vnm 曝光的 query 組成
  - 介於 1.0-2.0%：**部分命中**（需求真實但轉換弱於預期，可能是 Hub 策展或 metadata 問題）

### ~~EXP-2026-07-18-pt | 葡萄牙文出生後三源需求驗證~~ ✅ **命中（2026-09-17 heartbeat 到期日當天判）**

> **2026-09-17 判定**：pt 支系 07-18 出生，D+30〜D+60 窗口取 08-17〜09-16。`sc-query.py --filter country=bra`：**clicks 126 / impressions 21,357 / CTR 0.6% / 平均排名 7.7**。基線（2026-06 整月）：clicks 5 / impressions 4,217 / CTR 0.1%。
>
> **126 ≥ 50，命中**，而且是門檻的 2.5 倍。這條 EXP 賭的是「CF 邊緣請求在零 pt 內容下超過已有語系」這個訊號對語言選址有獨立預測價值——bra 的點擊從月 5 → 126（25 倍）證明它不是純爬蟲假訊號。依原設計，**CF 邊緣維度寫進 EVOLVE 語言層方法論**：下一次語言選址（[evolve-2026-07-18-language-branches.md](../../reports/evolve-2026-07-18-language-branches.md) 的候選表）把 CF 邊緣請求排名列為正式的第三源，權重不低於 SC 曝光。這一步是 EVOLVE-PIPELINE 的文件更新，本次心跳只落判定不改 pipeline（另開 Write mode 做）。
>
> **跟 vi 對讀**：兩條同日出生的支系，pt 的 CTR（0.6%）比 vi（1.1%）低但點擊絕對量達標，vi 的 CTR 較高但沒到 2%。兩邊共同的形狀是曝光都長了 5〜6 倍——新語言一上線，Google 給的曝光比預測的多得多，CTR 型的預測值要把這個分母膨脹算進去。

> ~~機械到期檢查：due_date: 2026-09-17 | EXP-2026-07-18-pt~~（已判定，marker 除役）

- **預測**：pt 支系 P0 批上線後 60 天，**bra 市場 SC clicks 從基線 ~5/月量級 → ≥ 50/月**
- **根據**：pt 是四語中唯一「三源全確認」的缺口（SC 6,659 曝光 CTR 0.1% + CF 邊緣請求第 6 名 + GA 88 users）。CF 邊緣請求在零 pt 內容下超過已有完整語系的 fr/ko，是「爬蟲＋人類雙重需求」的獨立訊號
- **驗證指令**：`python3 scripts/tools/sc-query.py --dims country --filter "country=bra" --start <D+30> --end <D+60>`
- **反駁條件**：
  - D+60 bra clicks ≥ 50：**命中**（CF 邊緣訊號對語言選址有獨立預測價值，寫進 EVOLVE 語言層方法論）
  - D+60 仍 < 15：**反駁**——CF 邊緣請求是純爬蟲假訊號，人類需求不存在；CF 維度在語言選址的權重要下修
  - 介於 15-50：**部分命中**（需求真實但爬升慢，葡語圈分散在巴西/葡萄牙/非洲，單一市場指標可能低估）

> **副線（id/hi 不設硬 EXP）**：id 選址靠人口槓桿 + 移工社群質性判斷（SC 訊號中等）、hi 靠先佔窗口戰略（滲透最弱），兩者的價值在長尾而非 60 天轉換，用 dashboard 翻譯覆蓋 + 該市場 SC 趨勢定性追蹤即可，不綁可證偽硬預測。

### ~~EXP-2026-07-17-G | 404 根因判斷的收斂驗證~~ ✅ **命中（2026-08-16 週體檢補判，兩條件皆過）**

> **2026-08-16 判定**：到期日 2026-08-07 過了 9 天，黃燈掛著沒人動手，本次週體檢 c3 節點名後當場用 EXP 自帶的兩支驗證指令跑完。(a) CF 7d 404 率 **4.34%**（基線 14.99%，門檻 ≤ 8%）→ 過，且低於門檻近一半。(b) 可解析家族 2026-08-14 當日合計 **662/日**（slug-variant 543 + untranslated-demand 95 + renamed-or-truncated 23 + cross-lang-slug 1），對基線 ~8,900/日是 **−92.6%**，門檻 ≥ 60% → 過。**「爬蟲忠實跟隨我們自己在 hreflang 公告的死 URL」這個因果判斷成立**；停止公告加 301 收尾之後，快取如預期在數週內衰減。CONSCIOUSNESS §適應性反應 的 🟠 CF 404 率高檔盤整 可降級。
>
> 副線紀錄：cross-lang-slug 家族當日只剩 **1** 筆（基線 538/日），這是 2026-07-25 上線的 827 條英文別名的效果，落在 EXP-2026-07-25-alias 條件 (a) 的 ≤ 50/日 之內——但那支 EXP 到期日是 2026-08-24，**本次不提前判定**（per REFLEXES #67，判定要在它自己的時間窗用它自己的指令做）。這裡只留一句：目前趨勢指向命中。
>
> ~~機械到期檢查：due_date: 2026-08-07 | EXP-2026-07-17-G~~（已判定，marker 除役）

- **原預測**：hreflang 死鏈根源修復（`f369f3c8e`）+ 資料驅動 redirect 部署後，(a) **D+21（2026-08-07）CF 7d 404 率從 14.99% 降到 ≤ 8%**；(b) D+30 monitor-404 的「可解析家族」（slug-variant + cross-lang-slug + untranslated-demand + renamed-or-truncated 合計）從基線 ~8.9k/日下降 ≥ 60%
- **根據**：3 天全量交叉對賬顯示 78.1% 的 404 是「疑似真實文章 URL」且 94% 是 bot——判斷為爬蟲忠實跟隨我們自己在 hreflang 公告的 13,014 條死 URL（28.3% 直接命中自吐集合，top-10k 截斷下的下界）。若判斷正確，停止公告 + 301 收尾後爬蟲快取會在數週內 decay。完整證據鏈：[reports/404-root-cause-2026-07-17.md](../../reports/404-root-cause-2026-07-17.md)
- **驗證指令**：`python3 scripts/tools/monitor-404.py --days 7`（看 families 分佈）+ dashboard-analytics.json 的 7d 404 rate
- **反駁條件**：
  - D+21 404 率 ≤ 8% 且可解析家族大幅下降：**命中**（因果判斷正確，🟠 適應性反應可降級）
  - D+21 仍 > 12%：**反駁**——「爬蟲跟隨自家死鏈」的權重判斷錯誤，另有大宗來源未被分類捕捉；回去讀 monitor 的 unknown 家族，i18n 反查 middleware 升優先
  - 介於 8-12%：**部分命中**（方向對、decay 比預期慢；多等 2 週再判，爬蟲重抓週期本有長尾）
...
| **EXP-2026-07-17-G** \| 404 根因收斂           | 2026-08-16 | ✅ **命中（過期 9 天後補判，兩條件皆過）**。CF 7d 404 率 4.34%（門檻 ≤ 8%，基線 14.99%）；可解析家族 662/日（門檻 −60%，實測 −92.6%）。hreflang 自公告死鏈的因果判斷成立                          | CONSCIOUSNESS §適應性反應 🟠 CF 404 可降級；alerts 黃燈可消。判定靠週體檢 c3 節點名，不是機械檢查會動手（#58 再驗） |
