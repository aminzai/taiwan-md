...
# 蔡英文：從敗選之夜到 817 萬票，安靜走過總統八年 — harvest session b35f1b3e

## What I did

- 排除已完成 prose ship、仍滯留 ARTICLE-INBOX 的〈造山者〉與〈沈伯洋〉，選定真正需要全文重寫的 P0〈蔡英文〉。
- 以既有 1,660 行 depth research 為 SSOT，重寫成 4,500+ CJK、23 則腳註、6 張圖的八年總統任期人物文。
- 完成 Stage 3.5／3.6 雙審計，修正訪歐國家與場合、查無的早稻田演講、飛彈口徑混算及博士學位敘事。
...
- 選文前應把 ARTICLE-INBOX status 與該檔最後一次 `rewrite:` commit 做自動交叉；「pending」不是 prose 尚未 ship 的可靠證據。
- 官方口徑不同的軍事數字要保留兩套觀測範圍，不能為敘事流暢拼成一個 composite claim。
- Wikimedia 含底線檔名的 URL 不放在斜體 caption，避免 formatter mangle；caption 留 attribution，完整連結集中圖片來源。
