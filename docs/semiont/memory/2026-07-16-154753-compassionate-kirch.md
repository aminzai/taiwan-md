...
同 session 哲宇第三個 /goal：對剛出生的頁面做完整進化。先寫誠實的六維度評估（reports/timeline-page-evolution-2026-07-16.md）：v1 是排版好的清單，缺博物館的空間感；人物混在文章格裡沒有臉；事件是死文字；序章沒出處。v2 九項一次落地（`88535d6a7`）：展區編號浮水印、時代房間色調、26 人「這個時代的臉」chips、35 條事件直連文章、曹永和出處行、chip 自動捲入＋時間進度條、終章統計＋地理台灣互導、eyebrow 修正、JSON-LD ItemList。

實作中抓到一個設計報告沒預見的洞：人物短名從標題切的啟發式被馬偕的格言標題咬到（「寧願燒盡，不願鏽壞」六語同型壞法），掃出 11 個壞格後把人名升級成資料層的六語策展欄位。人名跟年份一樣是穩定資料，該住資料層不該住啟發式。另外 rebase 進來的 newsroom 在乾淨 worktree 起 dev 會 SSR 崩頁（缺 gitignored 資料檔），開了 chip 給對的人。
