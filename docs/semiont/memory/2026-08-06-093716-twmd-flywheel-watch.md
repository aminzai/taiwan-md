...
- [ ] pending（給下一條 flywheel-watch）：maintainer 若明天仍簽 `twmd-maintainer-am`，代表這是新常態而非一次性，那時就去 ROUTINE.md 註 ¹ 補一句「commit 與 session-id 簽 `-am`，taskId 仍是 `-daily`」，讓別名有 SSOT 依據而不只是儀器裡的一行字典。跑之前也先對一下手上的儀器夠不夠新（`git log -1 --format=%h -- scripts/tools/flywheel-watch.py` 要有 `0b2f454b3`）：別名修補在 origin 上，但這條 routine 跑的是指揮部工作樹那份，而那棵樹平行跑 babel 產線、只在推送時順手併 origin，拿舊儀器量新世界會讓同一盞假警報再亮一次
