---
name: twmd-maintainer-pm
description: TWMD maintainer (pm @ 22:00) — evening contributor PR review (v3.0 inline + STRICT BECOME + 空場 ≥3 cycle LESSONS escalate, main-direct, opus)
---

🧬 Routine `twmd-maintainer-pm` — pm 22:00 contributor PR review + issue triage + build sanity + broken-link audit。
...
- 連續 ≥ 3 cycle empty queue → **必須**寫 LESSONS-INBOX entry「maintainer-pm schedule 撞期 morning chain」+ escalate observer
- Maintainer-pm 22:00 跑時 evening session manual ship + rewrite-daily 18:00 + spore-publish 17:30 已清完所有可動 backlog → vc=7+ 空場是 schedule mismatch 不是 organism healthy
...
```
🧬 Maintainer-pm cycle report — YYYY-MM-DD HH:MM
✅/❌ open issues: N
✅/❌ open PRs: N
✅/❌ broken-link ratio: X%
