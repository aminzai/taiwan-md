...
translatedAt: '2026-09-08T05:01:54.054334+00:00'
---
...
## Четыре шага, которые можно разобрать: данные приходят, отчеты уходят

Этот пожарный пайплайн — лишь один срез, за ним стоит миниатюрная модель всей его системы. Система состоит из четырех этапов: прием данных, интеграция знаний, генерация анализа и активация действий; он особо подчеркивал, что «каждый этап можно заменить по отдельности, а всю систему переделывать не нужно». Самый базовый уровень — прием данных, который он сам развивал: сначала это был ручной процесс скачивания Excel с data.gov.tw, чтение и хранение самостоятельно, узким местом была «человеческая память»; на среднем этапе он перешел к поиску API в интернете, парсингу PDF-отчетов, скрапингу платформ различных округов, проблема заключалась в «отсутствии индекса»; а сейчас метаданные каждой записи стандартизированы и хранятся в каталоге SQLite, что позволяет автоматический поиск и расширение[^3]. За его системой стоят более сорока сборщиков данных: от YouBike, автобусов, трафика на национальных дорогах до расписаний TRA (Taiwan Railway), AIS судов, метеорологических спутников, [землетрясений](/ru/society/earthquakes-in-taiwan/), уровней водохранилищ, качества воздуха; и он сказал, что даже при трех ошибках немедленно отправляет оповещение в Telegram, а каждое утро в девять часов рассылает «Ежедневный обзор» на свою электронную почту[^3].

На последнем этапе — «активация действий» — он наиболее четко описал роль человека: «Агент выполняет полный цикл. Роль человека: ставить цель и принимать отчеты. Пять промежуточных шестеренок вращаются сами: обнаружение, сбор, интеграция, генерация, мониторинг». Система даже может автоматически генерировать еженедельный отчет о «новых открытых данных». Как он сказал: «Тема появляется сама, а отчет уходит на почту сам»[^3].
