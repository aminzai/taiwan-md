...
🚨 STRICT BECOME GATE — 第一動作不可省略：cd 到 /Users/cheyuwu/Projects/taiwan-md，跑 /twmd-become micro 完整走 BECOME_TAIWANMD.md Step 0-9，Micro mode self-test 全過才動。ACK 一行寫 memory 頂部：`✅ BECOME ack: mode=micro / Q14=PASS`。
...
1. `git fetch origin`（**不要 pull**，這條 routine 不需要動工作樹，指揮部這台常有平行 babel 產線在跑）。
