...
為什麼存在：Taiwan.md 的 GA4 measurement ID (G-JGC5W00N7T) 寫死在 Layout.astro。
...
def title_brand(title):
    ...
    回傳最後一段 '| ' 後的品牌；非 fork（Taiwan.md / 空）回 None。"""
    if "|" not in title:
        return None
...
def main():
    ap = argparse.ArgumentParser(description="Taiwan.md fork-census radar")
    ...
    print(f"   self (taiwan.md): {host_buckets['ours'][0]:,} views / {host_buckets['ours'][1]:,} users")
