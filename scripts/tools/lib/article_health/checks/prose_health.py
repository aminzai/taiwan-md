...
# 「化學名詞變成全民動作」的「名詞」是實指名稱不是語法術語、編輯教學文合法討論「主詞換成」）。
# 收緊為只抓「語法術語 + 比較/等同」——描述兩個真實事物「動詞不一樣」才是把分析鷹架
# 留在正文裡。文學/語言類文章仍可能誤報，WARN 級可人工忽略。
...
_BACKSTAGE_DETECTORS = [
    (
        ...
        "分析框架洩漏",
        "用語法／文體術語描述真實人事物——那是作者拆解論點時用的鷹架，不是讀者讀得到的東西。"
        "把鷹架拆掉，只留畫面本身。",
    ),
    (
